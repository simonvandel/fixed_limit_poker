...
class SimonBot(BotInterface):

    def __init__(self, name="SimonBot"):
    ...
    def handlePreFlop(self, observation: Observation) -> Action:
        ...
        # A bit more conservative than percentbot
        if handPercent < .35:
            return Action.RAISE
        elif handPercent < .55:
            return Action.CALL
    ...
    def handleFlop(self, observation: Observation) -> Action:
        ...
        if handPercent <= .50:
            return Action.RAISE
        elif handPercent <= .75 or self.getFlushDraw(observation) or self.getStraightDraw(observation):
            return Action.CALL
        return Action.FOLD
